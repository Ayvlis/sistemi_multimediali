class Mascotte {
  ...
  public void display() { 
    ...
    pushMatrix();
    translate(0, -newR/20);
    float a = atan2(mouseY-(position.y -newR/5 +newR/20), mouseX-(position.x +newR/2) );
    if (mouseX<position.x +newR/2+5 && 
        mouseX>position.x +newR/2-5 && 
        mouseY<position.y -newR/5+5 && 
        mouseY>position.y -newR/5-5 ) {
      ellipse(mouseX-(position.x +newR/2), mouseY-(position.y -newR/5 +newR/20), newR/3, newR/3);
    } else {
      rotate(a);
    ...
    pushMatrix();
    translate(0, -newR/20);
    float b = atan2(mouseY-(position.y -newR/5 +newR/20), mouseX-(position.x -newR/2) );
    if (mouseX<position.x -newR/2+5 && 
        mouseX>position.x -newR/2-5 && 
        mouseY<position.y -newR/5+5 && 
        mouseY>position.y -newR/5-5 ) {
